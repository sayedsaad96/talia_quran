...
> **Rev 4 changelog (2026-08-23 — second external critique, all claims re-verified against code):** **CO-8 scope expanded (verified): hardcoded religious content exists in 4 more surfaces** — notification fallback duas (`notification_service.dart:49-53`), a hand-typed Quran verse on the certificate (الإسراء 9, `certificate_widget.dart:438`), a hardcoded dua in settings (`settings_notification_tiles.dart:730`), and **dua text truncated at 177 chars + "…"** in notifications (`notification_service.dart:838-843` — modified religious text, HAL-02). **New CO-10 (P1): `QuranTextDisplayFormatter.cleanAyahForMemorization` modifies ayah text in the memorization display path** (`quran_text_display_formatter.dart:20-55` via `qcf_hifz_verse_view.dart:159,263`): strips trailing ayah markers, rewrites punctuation spacing, **inserts spaces around the definite article**, collapses whitespace incl. ZWJ/ZWNJ — rendered memorization text is not character-for-character the dataset text (QUR-01/HAL-02). **CO-4 strengthened:** 73/85 references carry no resolvable number (was: duas only), and the `Zikr` entity (`azkar_entities.dart:5-30`) cannot represent grade/tier/sourceType at all. **New CO-11 (P2): kids grading reuses the adult 92% STT threshold** (`kids_mode_cubit.dart:80,239` — `V2RecitationEvaluator()` default), violating KB `12_islamic_education.md:20,30,45,57` ("keep Kids Mode grading more forgiving… clearly separated config; ❌ applying the same strict grading threshold to both"). **New CO-12 (P1): no dataset governance manifest** — KB `18_references.md` treats alquran.cloud as a *candidate to verify*, not an already-vetted source; no pinned version/license/riwayah/SHA-256 exists. Two critique claims **rejected as not KB rules** (avg-of-last-3 weak definition; recall-before-listen session mandate — absent from KB 06/07; recorded as product decisions, not violations). **Executable plan extracted to `docs/talia_remediation_plan.md`** (phases 0-4, tasks R-01…R-24).
...
| CO-8 | **Hand-typed Quran verses and unsourced hadith hardcoded in code — 5 surfaces (Rev 4, KB-critical).** (a) `azkar_page.dart:367-426` `_tips`: 9 verbatim/paraphrased hadith with no source/grade + 9 Quran verses typed by hand in ﴿…﴾ (lines 380-388); (b) `notification_service.dart:49-53` `_fallbackDailyDuas`: 3 hardcoded duas, no source; (c) `certificate_widget.dart:438`: hand-typed verse ﴿إِنَّ هَٰذَا الْقُرْآنَ يَهْدِي لِلَّتِي هِيَ أَقْوَمُ﴾; (d) `settings_notification_tiles.dart:730`: hardcoded dua in a settings subtitle; (e) `notification_service.dart:838-843`: `_compactNotificationText` **truncates dua text at 177 chars + "…"** — displayed religious text ≠ source text (HAL-02). **KB violations:** HAL-01/HAL-02/QUR-01 (verses must come from the vetted dataset, never hand-typed or truncated), HAD-01/HAD-03 (hadith without grade/source); KB `14_content_validation.md` prescribes a CI lint for exactly this pattern | High (Critical per KB risk table) | **P1** |
...
| CO-10 | **`QuranTextDisplayFormatter.cleanAyahForMemorization` modifies ayah text in the memorization display path (Rev 4).** `quran_text_display_formatter.dart:20-55`, used by `qcf_hifz_verse_view.dart:159,263`: strips trailing ayah-end markers/numbers, rewrites punctuation spacing, **`_normalizeMissingWordSpacing` inserts spaces before the definite article ال**, collapses all whitespace incl. ZWJ/ZWNJ/BOM. The text shown to the memorizing user is therefore not character-for-character the vetted dataset text — QUR-01 ("checked character-for-character against the vetted dataset") and HAL-02 ("no altering wording, diacritics, or verse boundaries"). If spacing fixes are genuinely needed for QCF rendering, they must live in a reviewed rendering layer with a byte-level diff test against the dataset — not an undocumented runtime rewrite | High | **P1** |
| CO-11 | **Kids grading reuses the adult 92% STT threshold (Rev 4).** `kids_mode_cubit.dart:80` instantiates `const V2RecitationEvaluator()` (default `kV2PassThreshold = 0.92`), comment at :239 confirms "mirroring the adult V2 path (≥ 92% similarity)". KB `12_islamic_education.md` requires kids grading to be **more forgiving** and kept as **explicitly separated config** (:20 more forgiving; :30 don't unify thresholds "for consistency"; :45 clearly separated explicit config; :57 ❌ applying the same strict grading threshold to both) | High | **P2** |
| CO-12 | **No dataset governance manifest (Rev 4).** `quran.json`/`azkar.json` have no pinned source URL+edition, version, license, riwayah declaration, SHA-256, import date, or human review status. KB `18_references.md` treats alquran.cloud as a *candidate to verify at integration time* (licensing, methodology, versioning), not an already-vetted source; its Engineering Implications require version-pinning datasets "the same way dependencies are pinned". A pinned manifest + reviewer sign-off record is the KB's prescribed mechanism | High | **P1** |
